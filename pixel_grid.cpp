...
//*******************************************
//The function that runs your rain sequence
void rain_sequence(void)
{
  //**********************************************
  //Check if enough time has passed between frames
  if(time_since(begin) < rain_animation_framerate)
  {
    return;
  }
  ...
  for(uint8_t i=0; i<grid.numPixels(); i++)
  {
    grid.setPixelColor(pixel_map[i],rain_animation_lut[frame][i]);
  }
  grid.show();

  //*********************************************************************
  //Set the animation to loop by starting from the beginning once it ends
  if(++frame >= rain_animation_length)
  {
    frame = 0;
  }
